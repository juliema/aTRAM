...
foreach my $contigfile (@contigfiles) {
	...
	if ($contigfile =~ /(.+)\.(.+)\.fasta/) {
		...
		system_call (Configuration::find_bin('blastn') . " -query $filepath -db $blastdb -num_threads $processes -evalue 1e-50 -out $blastfile -outfmt '6 sseqid qseqid slen evalue length'", $log_file);
		my $matchmatrix = {};
		my @matrixkeys = ();
		open FH, "<", $blastfile;
		my $line = readline FH;
