...
def select_files(cxn, ref_name, taxon_name):
...
def create_exonerate_table(cxn):
    ...
    # TODO: Figure how to remove the need for the UNIQUE INDEX
    cxn.executescript("""
        ...
        CREATE UNIQUE INDEX exonerate_dup_idx
            ON exonerate (ref_name, taxon_name, contig_name, beg, end);
        """)


def insert_exonerate_results(cxn, batch):
    """Insert a batch of exonerate result records into the database."""
    # TODO: Remove the need for the OR REPLACE
    if batch:
        sql = """
            INSERT OR REPLACE INTO exonerate (
                ref_name, taxon_name, contig_name, beg, end, target_seq)
            VALUES (
                :ref_name, :taxon_name, :contig_name, :beg, :end, :target_seq);
            """
        cxn.executemany(sql, batch)
...
def select_next(cxn, ref_name, taxon_name, beg=-1):
    ...
    It's looking for the closes contig to the given beginning  The tiebreaker
...
#
#
# def keep_top_contig(cxn, ref_name, taxon_name):
...
#          LIMIT 1;
#         """
#     cxn.execute(sql, (ref_name, taxon_name))
#     cxn.commit()
#
#
# def keep_contig(cxn, ref_name, taxon_name, contig_name):
#     """Flag contig as a keeper."""
#     sql = """
#         UPDATE exonerate
#            SET keep = 1
#          WHERE ref_name    = ?
...
#            AND contig_name = ?;
#        """
#     cxn.execute(sql, (ref_name, taxon_name, contig_name))
#     cxn.commit()
#
#
...
#          WHERE ref_name   = ?
#            AND taxon_name = ?
#         ORDER BY ref_name, taxon_name, beg, end DESC;
#         """
